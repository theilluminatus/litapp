'use strict';
// Based on cors-escape/lib/cors-escape.js but with some edits

const debug = require('debug')('cors-escape');
const httpProxy = require('http-proxy');
const url = require('url');
const getProxyForUrl = require('proxy-from-env').getProxyForUrl;
const {isValidHostName, parseURL} = require('./cors-escape/lib/helpers');
 ...
 */
function setCookies (proxyRes, res) {
    const stripLocation = (v) => {
        if (typeof v === "string") {
            return v.split("domain=")[0]
        } else {
            return v.map(i => i.split("domain=")[0]);
        }
    };

    const headers = ['set-cookie', 'set-cookie2'];

    headers.forEach(h => {
        if (proxyRes.headers[h]) {
            const oldValue = res.getHeader(h) || [];
            let newValue;
            if (typeof oldvalue === "string") {
                newValue = oldValue + ", " + stripLocation(proxyRes.headers[h]);
            } else {
                newValue = oldValue.concat(stripLocation(proxyRes.headers[h]));
            }
            res.setHeader(h, newValue);
        }
    });
}
...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        if (corsEscape.targetWhitelist.findIndex(h => location.href.indexOf(h) > -1) < 0) { // Edited
        ...
        if (!corsEscape.originWhitelist.includes(origin)) { // Edited
            res.writeHead(403, 'Forbidden', corsHeaders);
